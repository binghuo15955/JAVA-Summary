...
- 使用java.lang.reflect包的方法==对类进行反射调用==
- 当初始化一个类，如果==其父类没有被初始化，则先会初始化它的父类==
...
- 通过反射，获得一个类的==实例对象==。`newInstance()`
- 通过反射，调用类的==方法==。`invoke()`
- 通过反射，访问类==成员变量==。如果是private修饰的，可以使用`setAccessible(true)`来关闭权限检测。
