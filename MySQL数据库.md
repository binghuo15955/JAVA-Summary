...
**==M==ultiple ==V==ersion ==C==oncurrent ==C==ontrol多版本并发控制**，目前只能在==Innodb==下(MyIsAM不支持事务)，只在==RC==和==RR==两个隔离级别下工作。
...
1、保证事务rollback的原子性和一致性。对==数据进行恢复==；

2、用于==MVCC快照读的数据==，在MVCC中，通过读取undo log的历史版本数据可以实现不同事务版本号都拥有自己的独立快照数据版本。
...
- **Read Committed**：==每次==快照读生成并获取最新的Read View；
- **Repatable Read**：==同一个事务==的==第一个==快照读创建Read View，**MVCC（处理读写冲突）+ 间隙锁（处理写写冲突）防止幻读**
...
**Note**：主从节点使用的==binlog文件+position偏移量==来定位主从同步的位置，==增量同步==！
